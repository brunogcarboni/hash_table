...
std::ostream& operator<<(std::ostream& outs, const Posicao & p)
{
    if (!p.posicaoLista.has_value())
        return outs << "PosicaoTabela : " << p.posicaoTabela << " TamanhoLista: " << p.tamanhoLista;
    else
        return outs << "PosicaoTabela : " << p.posicaoTabela << " TamanhoLista: " << p.tamanhoLista << " posicaoLista :" << p.posicaoLista.value();
}
